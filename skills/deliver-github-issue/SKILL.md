...
description: Deliver one GitHub issue through review, implementation, verification, and a review-ready PR.
---
...
4. Execute dependency-ready items. Isolate only concurrently disjoint work; never pre-create dependent worktrees. Start dependants after prerequisites integrate from exact new `HEAD`. Run `$implement-reviewed-item`, integrate clean commits stably, and update fingerprints. Later in-scope defects in committed items become minimal fix items; preserve/restart dependants from resulting `HEAD`; never rewrite reviewed history.
...
7. Run `$create-or-update-pr mode=draft`, then `$pr-review` → `$pr-fixer` incrementally until a fresh final exact-head review is `CLEAN`; preserve questions.
