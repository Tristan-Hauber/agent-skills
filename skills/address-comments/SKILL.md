...
1. Run `$evaluate-github-comments` with any verified receipt; preserve comment IDs, authors, fingerprints, and all `Q#`/`P#`/`N#` dispositions. The evaluator automatically runs `$github-review-learnings`; preserve its note result alongside the review receipt.
