---
name: implement-task
description: Implement one bounded task on the intended branch and create one reviewed final commit.
---
...
4. Run `$implement-reviewed-item` with the evidence packet; it keeps work uncommitted until final `CLEAN` and validation, then creates exactly one commit.
